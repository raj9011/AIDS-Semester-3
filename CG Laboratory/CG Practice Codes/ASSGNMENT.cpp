...
{  
int i;  
cout<<"\n Enter Number Of Vertices Of Polygon: ";  cin>>v; if(v>2)  
{  
for(i=0;i<v; i++)  
{  
...
s,s2; do  
{  
s=ymin+0.01; delay(50);  cleardevice();  
while(s<=ymax)  
{ ints(s);  
sort(s);  
s++;  
...
int x1,x2,y1,y2,temp;  c=0; for(int  
i=0;i<v;i++)  
{  
x1=p[i].x; y1=p[i].y;  
x2=p[i+1].x; 
...
cleardevice(); poly x;  
x.read();  
x.calcs(); cleardevice(); cout<<"\n\tEnter The Color You Want  :(In Range 0 To 15 )->"; cin>>cl; setcolor(cl);  
x.display();  
getch();
 return 0;
}
